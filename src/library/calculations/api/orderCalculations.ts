...
import { calculateOrderItemBasePrice, calculateOrderItemModifiersPrice } from "../common/orderCommonCalculations";
import {
...
export const calculateOrderCheckItemInclusiveTaxApi: CalculateOrderCheckItemInclusiveTaxApi =
  ({ orderCheckItemBasePrice, orderItemTaxes }) => {
    ...
    const inclusiveTax = (orderCheckItemBasePrice * taxDecimal) / (1 + taxDecimal);
...
export const calculateOrderCheckItemBasePriceApi: CalculateOrderCheckItemBasePriceApi =
  ({ orderItem, orderCheckItem }) => {
    ...
    const orderCheckItemPrice =
      (orderItemBasePrice + orderItemModifiersPrice - orderItemDiscountPrice) * orderCheckItemAmount;
...
// ==================================================== //
interface CalculateOrderCheckItemChargeBasePriceApiParams {
  orderItem: Partial<OrderItem_V2>;
  orderCheckItem: Partial<OrderCheckItem_V2>;
}
type CalculateOrderCheckItemChargeBasePriceApi = (
  params: CalculateOrderCheckItemChargeBasePriceApiParams
) => number;

export const calculateOrderCheckItemChargeBasePriceApi: CalculateOrderCheckItemChargeBasePriceApi =
  ({ orderItem, orderCheckItem }) => {
    const { amount } = orderCheckItem;
